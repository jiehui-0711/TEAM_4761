...
public class runb {
    ...
    public static void main(String args[]) throws IOException {
        String inp_path = "data1/t3.tsv";
        String out_path = "out.txt";
        String out1_path = "out1.txt";
        String out2_path = "out2.txt";
        toset();
        BufferedReader br = new BufferedReader(new FileReader(inp_path, StandardCharsets.UTF_8));
        BufferedWriter out = new BufferedWriter(new FileWriter(out_path, StandardCharsets.UTF_8));
        BufferedWriter out1 = new BufferedWriter(new FileWriter(out1_path, StandardCharsets.UTF_8));
        BufferedWriter out2 = new BufferedWriter(new FileWriter(out2_path, StandardCharsets.UTF_8));
        TreeSet<String> dus = new TreeSet<>();
        TreeSet<String> dus1 = new TreeSet<>();
        file_ind = 0;
        words = -1;
        ls = 0;
        int www = 0;
        ...
        System.out.println(file_ind);
        while ((line = br.readLine()) != null) {
            String line1[] = line.split("\t");
            if(line1[0].charAt(0)=='H') continue;
            ArrayList<String> ss;
            if (line1[0].charAt(0) != 'f') {
                file_ind = Integer.parseInt(line1[0]);
                words = Integer.parseInt(line1[1]);
                ls = words < 2 ? 0 : ls + 1;
                ...
                if (file_ind != Integer.parseInt(line1[0].substring(4, line1[0].length()))) {
                    bo_stre = true;
                }
                file_ind = Integer.parseInt(line1[0].substring(4, line1[0].length()));
                words = Integer.parseInt(line1[1]);
                String s = age(line);
    ...
    public static void toset() throws IOException {
        ...
        String bad_doc[] = {"TO", "EE", "MC", "LK", "LR", "LN", "ZH", "JL", "KT", "KJ", "JT", "RR", "ED", "SC", "CA", "LJ", "JS", "DN", "YL", "NM", "KM", "JB", "AP", "LM", "HM", "FP", "PG", "MS", "SW"};
        ...
        String sp_doc[] = {"Ta", "cc", "ec", "mm", "ta", "cy", "bl", "rt", "vo"};
        for (String i : sp_doc) {
            set_doc.add(i);
        }
    }
    ...
    //--------------date--------------------------
    static String month[][] = {{"0", "0"}, {"Jaurary", "Jan"}, {"Faburary", "Fab"}, {"March", "Mar"}, {"April", "Apr"}, {"May", "May"}, {"June", "Jun"}, {"July", "July"}, {"August", "Aug"}, {"September", "Sep"}, {"October", "Oct"}, {"November", "Nov"}, {"December", "Dec"}};
    ...
    public static ArrayList<String> date(String n) {
        ...
        //---------------21.6.2023 
        if (n.contains("Tumour Board meeting on 27.8.62.  Th")) {
            n = n;
        }
        n1 = n.toCharArray();
        String days[] = {"", "", ""};
    ...
    public static String duration(String n, String file) {
        ...
        char n1[] = n.toCharArray();
        String sp[] = {"month", "months", "year", "yr", "yrs", "years", "week", "weeks", "wks", "wk"};
        for (String i : sp) {
            if (n.contains(i)) {
                ...
                return (file + "\tDURATION\t" + words + "\t" + words + "\t" + n);
            }
        }
    ...
    public static String street(String n, int ws) {
        ...
        String np[] = n.split(" ");
        for (String i : np) {
            if (i.length() > 0 && !bengset.contains((int) i.charAt(0))) {
                return "nono";
            }
        }
        String fs[] = {"HISTOPATHOLOGY REPORT", "MSI SUPPLEMENTARY", "SEALS HISTOPATHOLOGY REPORT", "MSI IMMUNOHISTOCHEMISTRY", "Dr ", "pelvic"};
        for (String i : fs) {
            if (n.contains(i)) {
                return "nono";
            }
